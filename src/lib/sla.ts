 ...
 * anything else.
 */
export function inWindow(deal: Pick<Deal, 'stage' | 'createdAt'>, now: Date): boolean {
  if (deal.stage !== 'inquiry') return false
  const age = now.getTime() - new Date(deal.createdAt).getTime()
...
export function slaState(
  ...
  const started = new Date(deal.createdAt)
